...
if st.session_state.seccion == 'menu':
    ...
    if st.button("🚀 CALCULAR ATRASOS"):
        ...
        if cambio == "No":
            ...
            res_txt = f"Categoría única: {cat_atr}"
        else:
            ...
            total = a1 + a2
            res_txt = f"Cambio de categoría gestionado"

        st.markdown(f"""
            <div class="card-anio" style="border-left:8px solid #f59e0b;">
                <p class="label-dato">{res_txt}</p>
                <p class="label-dato">TOTAL ATRASOS BRUTOS</p>
                <span class="val-new">{total:,.2f} €</span>
            </div>
        """, unsafe_allow_html=True)
        
        st.download_button("💾 GRABAR ATRASOS (.TXT)", f"Atrasos de {st.session_state.nombre}\nTotal: {total:,.2f}€\nHasta: {mes_h}", file_name=f"atrasos_{st.session_state.nombre}.txt")
