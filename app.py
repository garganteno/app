...
# Estilo de Alto Impacto (Original con mejoras de contraste)
st.markdown("""
    <style>
...
if st.session_state.seccion == 'menu':
    ...
    if st.button("🚀 CALCULAR"):
        ...
        for anio in range(2026, limite + 1):
            ...
            total_conv += (n_anual + p_u)
            total_sin += (p_act * h_an_new)
            ...
            st.markdown(f"""
                <div class="card-anio">
                    ...
                    <div class="grid-datos">
                        ...
                        <div class="col-dato"><p class="label-dato">Sueldo Anual</p><span class="val-old">Antes: {a_prev:,.2f} €</span><span class="val-new">{n_anual:,.2f} €</span></div>
                    </div>
        ...
        if vista == "COMPLETO":
            ...
            st.markdown(f"""
                <div class="card-balance">
                    ...
                    <p style="color:#10b981; font-size:16px; font-weight:bold;">MEJORA: +{dif_total:,.2f}€</p>
                </div>
            """, unsafe_allow_html=True)
            informe_txt += f"\nBALANCE FINAL:\n- Ganancia con convenio: {total_conv:,.2f}€\n- Ganancia sin subida: {total_sin:,.2f}€\n- Mejora Neta: {dif_total:,.2f}€"

        st.download_button("💾 GRABAR INFORME (.TXT)", informe_txt, file_name=f"informe_{st.session_state.nombre}.txt")
        if st.button("⬅️ VOLVER AL MENÚ", key="back_bottom"): st.session_state.seccion = 'menu'; st.rerun()
    ...
    # (Mantenemos la lógica de atrasos que ya tenías funcionando)
