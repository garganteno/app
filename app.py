...
if st.session_state.seccion == 'menu':
    ...
    with st.expander("⚙️ DATOS DE ATRASOS", expanded=True):
        ...
        if cambio == "Si":
            mes_c = st.selectbox("Mes del cambio:", MESES); cat_n = st.selectbox("Nueva Categoría:", list(TRAMOS_BASE.keys()))
    ...
    if st.button("🚀 CALCULAR ATRASOS"):
        ...
        if cambio == "No":
            p_n = motor_2026(p_ant_atr, h_sem_atr, cat_atr); total = (p_n - p_ant_atr) * h_mens * idx_h
        else:
            idx_c = MESES.index(mes_c)
            p_n1 = motor_2026(p_ant_atr, h_sem_atr, cat_atr); m1 = min(idx_h, idx_c); a1 = (p_n1 - p_ant_atr) * h_mens * m1
            a2 = 0.0
            if idx_h > idx_c:
                p_n2 = motor_2026(p_ant_atr, h_sem_atr, cat_n); m2 = idx_h - idx_c; a2 = (p_n2 - p_ant_atr) * h_mens * m2
            total = a1 + a2
    ...
    if st.button("🚀 CALCULAR PROYECCIÓN"):
        ...
        # Variables para Balance Final
        total_ganado_nuevo = 0.0
        total_ganado_antiguo = 0.0
        ...
        for anio in range(2026, limite + 1):
            ...
            # Acumulados para el balance
            total_ganado_nuevo += (n_anual + p_u)
            total_ganado_antiguo += (p_act * h_an_new) # Comparamos contra la nueva jornada pero precio viejo
        ...
        if vista == "COMPLETO":
            dif_total = total_ganado_nuevo - total_ganado_antiguo
            pct_total = (dif_total / total_ganado_antiguo) * 100
            st.markdown(f"""
                <div class="card-balance">
                    <p style="color:#10b981; font-size:16px; font-weight:bold; text-transform:uppercase;">⚖️ BALANCE TOTAL VIGENCIA (2026-2029)</p>
                    <p style="color:#94a3b8; font-size:14px; margin-top:10px;">Con precio hora antiguo ({p_act:.2f}€) ganarías:</p>
                    <p style="color:#ffffff; font-size:20px; font-weight:700;">{total_ganado_antiguo:,.2f} €</p>
                    <p style="color:#94a3b8; font-size:14px; margin-top:5px;">Con el nuevo convenio ganarás:</p>
                    <p style="color:#10b981; font-size:28px; font-weight:900;">{total_ganado_nuevo:,.2f} €</p>
                    <p style="color:#ffffff; font-size:16px; margin-top:10px;">Mejora total acumulada: <span style="color:#10b981;">+{dif_total:,.2f} € ({pct_total:.2f}%)</span></p>
                </div>
            """, unsafe_allow_html=True)
