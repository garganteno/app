...
st.set_page_config(page_title="Evolución Salarial Pro", layout="wide")
...
st.markdown("""
    ...
    .val-new { color: #ffffff; font-size: 19px; font-weight: 700; display: block; }
...
with st.sidebar:
    ...
    p_actual_input = st.number_input("Precio Hora Actual (€)", value=10.0, format="%.2f")
...
if st.button("GENERAR INFORME DE EVOLUCIÓN"):
    # Tramos Base 2026 (Suben 3% anual acumulado)
    tramos_2026 = {
    ...
    # Definir rango de años según selección
    ...
    for anio in anios_rango:
        ...
        # Subida de tramos: 3% anual acumulado (en 2026 es base 1.0)
        inc_tramos = 1.03 ** (anio - 2026)

        salario_base_calculo = p_acumulado * h_anuales
        salario_con_fijo = salario_base_calculo * fijo
        
        n_anual, p_u, encontrado = 0, 0.0, False
        for t in tramos_2026[cat]:
            t_actualizado = (t * inc_tramos) * f_jornada
            if salario_con_fijo < t_actualizado:
                n_anual = t_actualizado
                encontrado = True
                break
